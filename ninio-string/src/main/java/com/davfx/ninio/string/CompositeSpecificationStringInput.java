...
final class CompositeSpecificationStringInput<T> implements StringInput<T> {
	...
	public CompositeSpecificationStringInput(StringInput<T> globalElementSeparator, StringInput<T> specificationSeparator, StringInput<T> keyValueListSeparator, StringInput<T> keyValueSeparator, StringInput<T> trim, StringInput<T> wrappee) {
	...
	public String get(T h) {
		...
		return Joiner.on(ges).join(c.specification());
	}
}
