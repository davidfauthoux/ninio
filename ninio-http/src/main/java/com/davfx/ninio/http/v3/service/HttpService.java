...
import com.davfx.ninio.http.v3.HttpContentSender;
import com.davfx.ninio.http.v3.HttpHeaderKey;
import com.davfx.ninio.http.v3.HttpListeningHandler;
...
import com.davfx.ninio.http.v3.service.HttpController.HttpAsyncOutput;
...
public final class HttpService implements HttpListeningHandler, Disconnectable, AutoCloseable {
	...
	private static final int DEFAULT_THREADS = CONFIG.getInt("ninio.http.service.threads");
	private static final int STREAMING_BUFFER_SIZE = CONFIG.getBytes("ninio.http.service.stream.buffer").intValue();
	private static final double STREAMING_EPSILON = ConfigUtils.getDuration(CONFIG, "ninio.http.service.stream.epsilon");
	...
	public static Builder builder() {
		return new Builder() {
			private final ImmutableList.Builder<FilterHandler> handlers = ImmutableList.builder();
			...
			public HandlerBuilder register(HttpServiceHandler handler) {
				final FilterHandler h = new FilterHandler();
				h.handler = handler;
				handlers.add(h);
	...
	private final ImmutableList<FilterHandler> handlers;
	
	private HttpService(int threads, ImmutableList<FilterHandler> handlers) {
