...
@Deprecated
public final class OldPingClient implements Closeable {
	private static final Logger LOGGER = LoggerFactory.getLogger(OldPingClient.class);

	private final OldPingClientConfigurator configurator;
	private final Set<InstanceMapper> instanceMappers = new HashSet<>();
	private volatile boolean closed = false;

	public OldPingClient(final OldPingClientConfigurator configurator) {
					...
					public void handle(Address address, ByteBuffer buffer) {
						...
						for (int i = 0; i < numberOfRetries; i++) {
							...
							if (replyTime < 0) {
								...
								statuses[i] = OldPingClientHandler.VALID_STATUS;
							}
						}
	...
	private static final class Instance {
		private OldPingClientHandler.Callback.PingCallback callback;
		...
		public Instance(OldPingClientHandler.Callback.PingCallback callback, PingWriter write, PingableAddress pingedAddress, int numberOfRetries, double timeBetweenRetries, double retryTimeout) {
		...
		public void closedByPeer() {
			...
			OldPingClientHandler.Callback.PingCallback c = callback;
			callback = null;
