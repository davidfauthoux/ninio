...
public class PingTest {
	...
	public static void main(String[] args) throws Exception {
		...
		try (Ninio ninio = Ninio.create(); Timeout timeout = new Timeout()) {
			...
			try (PingClient client = ninio.create(PingClient.builder().with(RawSocket.builder().failing(new LockFailing(lock))).with(new ThreadingSerialExecutor(PingTest.class)))) {
