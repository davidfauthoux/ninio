...
public final class InternalSqliteCacheServerSnmpInterpreter implements InternalSqliteCacheServerReadyFactory.Interpreter<Integer> {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(InternalSqliteCacheServerSnmpInterpreter.class);
	
	public InternalSqliteCacheServerSnmpInterpreter() {
	}
	
	@Override
	public InternalSqliteCacheServerReadyFactory.Context<Integer> handleRequest(ByteBuffer packet) {
		try {
					...
					ber.beginReadSequence();
					{
						while (ber.hasRemainingInSequence()) {
							ber.beginReadSequence();
							{
								...
								return new InternalSqliteCacheServerReadyFactory.Context<Integer>(key, requestId);
							}
							// ber.endReadSequence();
						}
					}
	...
	public ByteBuffer transform(ByteBuffer packet, Integer requestId) {
		...
		for (Result r : results) {
			oidSequence.add(new SequenceBerPacket(BerConstants.SEQUENCE)
				.add(new OidBerPacket(r.getOid()))
				.add(new BytesBerPacket(BerPacketUtils.bytes(r.getValue()))));
		}
